...
const renderAdd = index =>{
    ...
    $("#add-todo-button").text("Add Todo").css({'background-color': 'rgb(110, 110, 110)'});
...
const renderDelete = todoBox =>{
    ...
    todoBox.animate({left: + '' + todoBox.width() + 'px', width: '0px'}, 
    animationTime, 
    () => todoBox.remove());//callback function
}
...
$("#todo-title-input").on('focus', function(){$(this).css({"border-color": "rgb(110, 110, 110)"});});
$("#date-input").on('focus', function(){$(this).css({"border-color": "rgb(110, 110, 110)"});});
$("#todo-description-input").on('focus', function(){$(this).css({"border-color": "rgb(110, 110, 110)"});});

//Controller--------------------------------------------------------------------------

//when the add todo button is clicked
$("#add-todo-button").off().on('click', function(){
    ...
    if(inputTitle.val() == '' || inputDate.val() == '' || inputDescription.val() == ''){
        renderWarning(inputTitle.val() == '', inputDate.val() == '', inputDescription.val() == '');
        return;
    }
